...
<html>
    <head>
        ...
        <script type="text/javascript">
            function fillForm(memberID, attendanceDate, checkInTime, checkOutTime, branchID) {
                document.getElementById('selectedMemberID').value = memberID;
                document.getElementById('selectedAttendanceDate').value = attendanceDate;
                document.getElementById('selectedCheckInTime').value = checkInTime;
                document.getElementById('selectedCheckOutTime').value = checkOutTime;
    ...
    <body>
        <div class="centered-container">
            ...
            <nav class="navbar">
                ...
                <a href="attendances.php" class="active">Attendances</a>
        ...
        <form method="POST" action="attendances.php">
            <div class="form-row">
            ...
            <div class="form-row">
                ...
                <input type="text" id="selectedAttendanceDate" name="selectedAttendanceDate" class="form-input">
            </div>
            <div class="form-row">
                <label class="form-label">CheckIn Time</label>
                <input type="text" id="selectedCheckInTime" name="selectedCheckInTime" class="form-input">
            </div>
            <div class="form-row">
                <label class="form-label">CheckOut Time:</label>
                <input type="text" id="selectedCheckOutTime" name="selectedCheckOutTime" class="form-input">
            </div>
            <div class="form-row">
        ...
        <hr />
        <h2 style="margin-left: 20px">Table - Attendances</h2>
        <?php
        ...
        function displayAttendances() {
            ...
            $result = executePlainSQL("SELECT * FROM Attendances");
            echo "<table border='1' style='margin-left: 20px'>";
            echo "<tr><th>MemberID</th><th>Attendance Date</th><th>CheckIn Time</th><th>CheckOut Time</th><th>BranchID</th></tr>";
        
            while ($row = oci_fetch_array($result, OCI_BOTH)) {
                echo "<tr onclick='fillForm(\"" . $row["MEMBERID"] . "\", \"" . $row["ATTENDANCEDATE"] . "\", \"" . $row["CHECKINTIME"] . "\", \"" . $row["CHECKOUTTIME"] . "\", \"" . $row["BRANCHID"] . "\")'>";
            ...
            echo "</table>";
            echo "<p style='margin-left: 20px'>Click the attendance record that you want to edit.</p>";
        }        
