...
<html>
    <head>
        ...
        <script type="text/javascript">
            function fillForm(memberID, workshopID) {
    ...
    <body>
        <div class="centered-container">
            ...
            <nav class="navbar">
                ...
                <a href="registersfor.php" class="active">RegistersFor</a>
            </nav>
        </div>
        ...
        <form method="POST" action="registersfor.php">
            <div class="form-row">
                <label class="form-label">MemberID:</label>
                <input type="number" id="selectedMemberID" name="selectedMemberID" class="form-input">
            </div>
            <div class="form-row">
                <label class="form-label">WorkshopID:</label>
                <input type="number" id="selectedWorkshopID" name="selectedWorkshopID" class="form-input">
            </div>
        </form>
        <hr />
        <h2 style="margin-left: 20px">Table - RegistersFor</h2>
        <?php
        ...
        function displayRegistersFor() {
            global $db_conn;
        
            $result = executePlainSQL("SELECT * FROM RegistersFor");
            echo "<table border='1' style='margin-left: 20px'>";
            echo "<tr><th>MemberID</th><th>WorkshopID</th></tr>";
        
            while ($row = oci_fetch_array($result, OCI_BOTH)) {
                echo "<tr onclick='fillForm(\"" . $row["MEMBERID"] . "\", \"" . $row["WORKSHOPID"] . "\")'>";
            ...
            echo "</table>";
            echo "<p style='margin-left: 20px'>Click the registration record that you want to edit.</p>";
        }        
